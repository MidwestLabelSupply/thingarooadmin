...
export default function UpdateOrder({ modalData, setOrderModal }) {
  const { setIsLoggedIn, setClientModal } = useContext(GlobalDataContext);

  const [companyName, setCompanyName] = useState(
    modalData?.row?.companyName || ""
  );
  const [productName, setProductName] = useState(
    modalData?.row?.productName || ""
  );
  const [quantity, setQuantity] = useState(modalData?.row?.quantity || null);
  const [limitedEdition, setLimitedEdition] = useState(
    modalData?.row?.limitedEdition || false
  );
  ...
  const editOrder = async (e) => {
    ...
    const { response, error, unauthorized } = await ordersApiCaller.editOrder({
      reqBody: {
